...
const Skills = () => {
  ...
  return (
    <section ref={containerRef} className="py-40 md:py-64 px-6 md:px-16 lg:px-32 bg-[#0B0B0F] border-t border-white/5 relative z-10 w-full">
      <div className="max-w-[90rem] mx-auto flex flex-col md:flex-row gap-24 md:gap-32 w-full">
        
        <div className="w-full md:w-1/4 shrink-0">
          <h3 className="text-xs tracking-[0.2em] text-[#D4AF37] uppercase font-medium">Core Capabilities</h3>
        </div>
        ...
        <div className="w-full md:w-3/4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-20 gap-x-12">
          {skillGroups.map((group, idx) => (
            <div key={idx} ref={el => groupsRef.current[idx] = el} className="flex flex-col gap-8">
              <h4 className="text-xs tracking-[0.15em] text-neutral-600 uppercase pb-4 border-b border-white/5">{group.category}</h4>
              <ul className="flex flex-col gap-4">
                {group.items.map((item, i) => (
                  <li key={i} className="text-xl md:text-2xl font-light text-[#EAEAEA]">
                    {item}
                  </li>
                ))}
              </ul>
